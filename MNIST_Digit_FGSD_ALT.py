...
def adv_test_loop(dataloader, model, loss_fn):
    size = len(dataloader.dataset)
    num_batches = len(dataloader)
    test_loss, correct = 0, 0
...
"""
def fgsm_attack(image, epsilon, y):
    # Collect the element-wise sign of the data gradient
    pred = model(image)
    loss = loss_fn(pred, y)
    loss.backward(retain_graph=True)
...
for data, y in test_dataloader:
    perturbed_data = fgsm_attack(data, epsilon, y)
"""
adv_test_loop(test_dataloader, model, loss_fn)
